...
<template>
  <header class="relative min-h-16 lg:min-h-20 container px-4 sm:px-6 flex items-center">
    ...
    <ul class="flex-1 flex items-center justify-end gap-3 sm:gap-10">
      ...
      <li>
        <NuxtLink v-if="!user.id" to="/auth/login" class="btn__solid btn__solid-white">Log in</NuxtLink>
        <NuxtLink v-else to="/c" class="border border-white/10 rounded-full btn__solid gap-1.5">
          <img v-if="user.avatar_url" :src="user.avatar_url" alt="avt" class="w-6 h-6 rounded-full object-cover" />
          <span>{{ user.name }}</span>
        </NuxtLink>
      </li>
    </ul>
