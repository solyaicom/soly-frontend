  ...
  <div class="w-full row-center px-3" :class="{ 'justify-end ': item.role === 'user', 'mt-6': !showPreDate, 'mt-3': showPreDate }">
    <div class="w-[94%] row-center overflow-hidden" :class="{ 'justify-end': item.role === 'user' }">
      ...
      >
        ...
        <div class="flex-1 flex flex-col items-start" :style="{ maxWidth: item.role === 'assistant' ? 'calc(100% - 38px)' : '100%' }">
          <div class="flex-1 flex flex-col items-start max-w-full space-y-2" :class="{ 'bg-app-card2 p-3 rounded-[8px] w-full': isChannel }">
            <div
              v-if="item.data.reply_message"
              class="w-full row-center bg-[#404040] px-3 pr-1 py-1 rounded-[6px] mb-2 border-l-[3px] border-l-[#fff]"
            ...
            <ChatObservation v-if="item.data.observations" :observations="item.data.observations" :created_at="item.created_at" />

            <div v-if="(thinking || !item.completed) && item.role === 'assistant'" class="bg-[#323232d9] p-2 rounded-[8px]">
    ...
    <Dialog v-if="item.data.reply_message" v-model:open="openPreview">
      <DialogContent class="bg-[#141414] py-3 px-3 border-none max-h-[calc(100vh-100px)] overflow-scroll">
        <div class="flex-1 flex flex-col items-start" :style="{ maxWidth: item.role === 'assistant' ? 'calc(100% - 38px)' : 'none' }">
          <ChatObservation
            v-if="item.data.reply_message.data.observations"
            :observations="item.data.reply_message.data.observations"
            :created_at="item.data.reply_message.created_at"
          />

          <div class="markdown" :style="{ maxWidth: item.role === 'assistant' ? 'calc(100%-38px)' : 'none' }">
            <div
              v-if="item.data.reply_message.content"
              v-html="md.render(item.data.reply_message.content)"
              class="text-[#ececec] text-[16px] break-words text-start w-full"
              :class="{
                'text-[#efefef] mt-0  ': item.role === 'user',
